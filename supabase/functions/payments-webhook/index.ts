...
async function handleCheckoutCompleted(session: any, env: StripeEnv) {
  // Record one-time payments; subscriptions are tracked separately above.
  ...
  try {
    ...
    if (item?.price) {
      priceId = item.price.lookup_key
        || (item.price.metadata as any)?.lovable_external_id
        || item.price.id;
  ...
  await getSupabase().from("payment_orders").upsert(
    {
