...
BEGIN
  ...
  -- 3) Email-based legacy invited_users (kept for backward compatibility).
  -- Do not read invite.id until the record has been populated by a SELECT.
  IF initial_status = 'pending' THEN
    ...
    IF staff_code IS NOT NULL AND length(staff_code) > 0 THEN
      ...
      invite_found := invite.id IS NOT NULL;
    END IF;
    ...
    IF invite_found THEN
      ...
      ON CONFLICT (user_id) DO UPDATE
        SET reports_to = EXCLUDED.reports_to,
  ...
  -- 4) Simple signup code: no invite token, no email match, no role grant.
  IF initial_status = 'pending' THEN
...
EXCEPTION WHEN OTHERS THEN
  RAISE LOG 'handle_new_user failed for user %, email %, sqlstate %, message %', new.id, new.email, SQLSTATE, SQLERRM;
  RAISE;
END;
$function$;